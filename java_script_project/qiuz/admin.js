...
const retrieveAdminData = function(url, id = null){
    ...
    if(data[0].level >= 10){
        ...
        subMenumenus1.innerHTML = `<a href='super.html?id=${myId}&ex=${Examid}&r=EXAMS'>
        <li > AS&nbsp;SUPER&nbsp;ADMIN</li> </a>
        `
    }
...
const FetchExamsData = function(url, quest='yes'){
    const fetchData = async (url) => {
        await fetch(url)
        .then(res => {
             if(!res.ok){
            ...
            .then(data => {
            listExamsAdmin(data, quest)
            })
        }
        fetchData(url)                    
    }
    
const listExamsAdmin = function(data, quest='yes'){
    aExams=document.getElementById('aExams');
    aExams.innerHTML = `<option value='${0}'>select&nbsp;${coursesAs}</option>`
...
const setAdminExamId = function(load='yes'){
        isloading=document.getElementById('notice');
        isloading.innerHTML = "...loading..."
...
const setAdminExamIdRecords = function(load='yes'){
    isloading=document.getElementById('notice');
    isloading.innerHTML = "...loading..."
    Examid=document.getElementById('aExams').value;
    ExamName=myExamNames[Examid]
...
const candidatesRecords = function(){
    ...
    <ul> 
        ...
        <ul> 
        <li >
        <select id="aExams" onChange="setAdminExamIdRecords()">
        ...
        `
        populateClassList()
        FetchExamsData('http://localhost:8001/Exams', 'no')
    ...
    <h3 style="text-align: center;">${CLASSNAME}: ${ExamName}</h3>
...
const retrieveRecords = function(CLASSID){
    ...
    const fetchData = async (url) => {
        ...
        if(isClass == 0){
            body=document.getElementById('body'),
            body.innerHTML += `<p style="text-align: center;">Sorry, No record available for this ${coursesAs.toLowerCase()} , please try another ${coursesAs.toLowerCase()}! </p>`
            isloading.innerHTML = ""
        }
