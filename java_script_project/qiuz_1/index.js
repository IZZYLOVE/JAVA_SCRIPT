...
function home(){
    ...
    const asAdmin = function(){
        ...
        header.innerHTML = `
        <li onclick="asCandidate()">CANDITATE</li>
        `;
            ...
            .then(data => {
               ...
               while(data[i] && i < myArra.length && aUser == null){
                   ...
                   // console.log(datax)
                    ...
                    // redirect to quiz
                    location.href = `quiz.html?id=${datax.id}&ok=${datax.score}`; 
                    return{aUser}
                }
                i++;  
...
const useData2 = function(id){
   // alert('useData2 bbhh')
    ...
    let body = document.getElementById('body');
        ...
        <h3>YOU ARE DONE WITH THIS QUIZ</h3>`;
        body.innerHTML = `<label>THANK YOU FOR PARTICIPATING </label>`;
        root.innerHTML = `<p>Do have a nice day.</p>
                        <button onclick='home()'> HOME</button>`;
        ...
        const fetchData = async (url) => {
               await fetch(url)
                ...
                .then(data => {
                   ...
                   myResult.correct = data[0].correct;
                   myResult.wrong = data[0].wrong;
                   myResult.score = data[0].score;
                   console.log('myResult2 '+myResult.score);
            ...
            const startQuiz = function(){
               myTime= (new Date().getTime() + (10*60*1000)+(2*1000)),
